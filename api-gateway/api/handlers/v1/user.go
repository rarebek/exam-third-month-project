...
func (h *handlerV1) Login(c *gin.Context) {
	...
	resp, err := h.serviceManager.UserService().GetUserByEmail(ctx, &pb.Email{
		Email: email,
	})
	if err != nil {
...
func (h *handlerV1) CreateUser(c *gin.Context) {
	...
	resp, err := h.serviceManager.UserService().CreateUser(ctx, &pb.User{
		Id:           id,
		Name:         body.Name,
		Age:          int64(body.Age),
...
func (h *handlerV1) UpdateUser(c *gin.Context) {
	...
	result, err := h.serviceManager.UserService().UpdateUserById(ctx, &user)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ResponseError{
			Code:  ErrorCodeInternalServerError,
			Error: err.Error(),
...
func (h *handlerV1) DeleteUser(c *gin.Context) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*time.Duration(h.cfg.CtxTimeout))
	defer cancel()
	_, err := h.serviceManager.UserService().DeleteUser(ctx, &pb.UserId{
		UserId: id,
	})
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ResponseError{
...
func (h *handlerV1) GetAllUsers(c *gin.Context) {
	...
	response, err := h.serviceManager.UserService().ListUser(ctx, &pb.GetAllUserRequest{
